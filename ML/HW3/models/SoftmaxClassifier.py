...
class SoftmaxClassifier:
    ...
    def forward(self, x, y):
        ...
        pred_y = self._softmax(prob)
        softmax_loss = -1 / num_data * np.sum(np.log(pred_y[range(num_data), y]))
    ...
    def compute_grad(self, x, y, weight, prob):
        ...
        for j in range(num_data):
            ...
            h[np.arange(num_data), y] -= 1
            grad_weight = np.dot(x.transpose(), h)
    ...
    def _softmax(self, x):
        ...
        softmax = (exp_x.transpose() / exp_x.sum(axis = 1)).transpose()
